...
{	
	...
	if(empty($first_name) || empty($last_name) || empty($Email) || empty($password) || empty($Phone_Number)) {
		...
		//updating the table
		$result = mysql_query( "UPDATE teacher SET first_name='$first_name', last_name='$last_name', Email='$Email' , password=md5('$password') , Phone_Number='$Phone_Number' WHERE id=$id");
...
$result = mysqli_query("SELECT * FROM teacher WHERE id=$id");
